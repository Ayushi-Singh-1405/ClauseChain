...
```json
{
  ...
  "source": { "circular": "SEBI/HO/ITD-1/ITD_CSC_EXT/P/CIR/2024/113", "clause": "Cl. 4.3.1" },
...
- ~~Finalized CSCRF excerpt (~10–15 clauses covering VAPT + incident reporting)~~ **Done** — `data/cscrf-excerpt.txt`
- ~~Hand-annotated ground truth for those clauses~~ **Done** — `data/ground-truth.json` (2 obligations, pending extraction test)
- One "old version" of a clause, authored for the Diff Agent beat
- One evidence file (sample VAPT report) ready to upload live

**"Ingest new circular" button — what actually runs live:** clicking it triggers a real, non-mocked pipeline run — Ingestion → Segmentation → Extraction → Classification → Rule Compilation genuinely execute against the CSCRF excerpt in front of the jury. This isn't optional polish: PS2's submission tips explicitly require "a functional prototype rather than static designs or mockups," and a jury that asks "is that actually running?" needs the honest answer to be yes.

What *is* pre-seeded, and legitimately so: the **Circulars list** (§9, screen 2) can show a handful of *other*, previously-ingested circulars as background context — those don't need to be processed live, they're just there to make the register look like a working system with history, not a one-clause demo. The CSCRF excerpt itself — the one the jury watches get ingested and extracted in Step 1–2 — must run live and real.

**Rough cost/latency shape:** the live-run portion only needs the 2 highlighted clauses (VAPT + incident reporting) processed on stage — a handful of LLM calls, each a few seconds. Rehearse this specific run in Week 4 until it's reliable. The rest of the ~10–15 clause excerpt can be processed ahead of time and shown as already-ingested in the Register, so total live latency during the jury slot stays low. Big Pickle (free tier) keeps per-call cost effectively zero either way.
...
- **If both teammates are present at the jury round:** split the runbook role explicitly — one person drives the live app and narrates, the other watches for failures and is the one who triggers the video fallback / jumps to the right chapter, rather than the presenter trying to do both under pressure. Decide who takes which role during Week 4 rehearsal, not in the room.
...
|---|---|---|
| Two-person team split | **Not decided.** Fallback until it is: default to whoever's free picks up the next unclaimed item from §12's weekly checklist, so the absence of a formal split doesn't stall Week 1. | Revisit for real before Week 2 if this default starts causing collisions or gaps |
...
| Hand-annotate CSCRF ground truth | **Done** \u2014 `ground-truth.json` drafted for both demo obligations, `status: pending_extraction_test` until run against the real Extraction Agent | High \u2014 update to match/partial_match/mismatch once tested |
| Finalize CSCRF excerpt clauses | **Done** \u2014 `cscrf-excerpt.txt`: \u00a74.3/4.3.1\u20134.3.5 (VAPT) + Annexure-O Part B \u00a71/3.1/3.3 (incident reporting), real text pulled from SEBI/HO/ITD-1/ITD_CSC_EXT/P/CIR/2024/113 | \u2014 |
